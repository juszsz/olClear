...
<head>
    ...
    <title>Admin-Viewing Fees (BYCIT)</title>
        ...
        <div class="dash-content">
            <div class="overview">
                <div class="title">
                    ...
                    <span class="text">View Fees (BYCIT)</span>
                </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if the payment record for the student and fee type already exists
    $sql_check = "SELECT * FROM fees_info WHERE studentNo = '$studentNo' AND feeType = '$feeType'";
    $result_check = $conn->query($sql_check);

    if ($result_check->num_rows > 0) {
        $row = $result_check->fetch_assoc();
        // Update status if the payment record exists and the status is not 'Paid'
        if ($row["status"] != "Paid") {
            $sql_update = "UPDATE fees_info SET status = 'Paid' WHERE studentNo = '$studentNo' AND feeType = '$feeType'";
            ...
            echo "Payment already made for this student and fee type!";
        }
    } else {
        // Insert a new record with 'Paid' status
        $sql_insert = "INSERT INTO fees_info (studentNo, feeType, status) VALUES ('$studentNo', '$feeType', 'Paid')";
...
$sql = "SELECT DISTINCT studentNo, studentName, section, feeType, amountPaid, dueDate, datePaid, status FROM fees_info WHERE feeType = 'BYCIT_FEE'  ORDER BY studentName";
